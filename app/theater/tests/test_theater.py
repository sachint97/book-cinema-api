...
class TheaterViewTestCases(TestCase):
    """Testing for movie filter, show selection and seat availability"""

    def setUp(self):
        self.client = APIClient()

    def test_movie_by_city(self):
        """Test to filter movies by city."""
    ...
    def test_shows_by_movies(self):
        ...
        Show.objects.create(**show, movie=movie_obj)
        res = self.client.get(
            f"/api/theater/shows/?city={city['slug']}&movie={movie['slug']}"
        )
        self.assertEqual(res.status_code, status.HTTP_200_OK)

        res1 = self.client.get(f"/api/theater/shows/?city={city['slug']}")
        self.assertEqual(res1.status_code, status.HTTP_200_OK)

        res2 = self.client.get(f"/api/theater/shows/?&movie={movie['slug']}")
        self.assertEqual(res2.status_code, status.HTTP_200_OK)

    def test_seating_by_show_view(self):
        """Test to see all available seats."""
        city = {"name": "testing city", "slug": "testing-slug"}
        movie = {
            "title": "testing",
            "description": "testing description",
